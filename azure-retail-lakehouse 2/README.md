...
These are original screenshots from the successful Azure demo runs. They show synthetic data and the actual validation output.

### Baseline orchestration

The initial-load job completed setup, seeding, Bronze, Silver, Gold and validation tasks successfully.

![Successful baseline job graph](docs/images/baseline-pipeline.png)

### Advanced validation and replay

The advanced demo passed its assertions before and after a no-new-input replay; the checked output row counts remained unchanged.

![Advanced validation and replay checks passed](docs/images/advanced-validation.png)

### SCD Type 2 and streaming windows

C1's region history progresses from North to West to South. C2 has a current deletion tombstone. The finalized five-minute web windows contain 3, 1 and 2 events.

![Customer history and finalized streaming windows](docs/images/scd2-and-streaming-results.png)

### Incremental CDF audit

Four customer batches consumed eight source rows, including one rejected row. Per-batch valid counts are not globally unique event counts.
